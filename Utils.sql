/*
        Various SQL stored precedures, triggers, and functions for utility purposes
        Authour: Will Wendell

*/
----------------------------------------------------------------------------------------------------------------------------
ALTER TRIGGER [dbo].[trg_NewProductEmail]
   ON [dbo].[Product]
   AFTER INSERT
AS 
BEGIN
		DECLARE @productCD VARCHAR(20)

		SELECT @productCD = I.sProduct_CD
			FROM inserted I WHERE I.sProfit_Center_CD IN ('CBSPT', 'NUMIS') AND I.sCommodity_CD IN ('G','S','P','L') 

		IF(@productCD IS NOT NULL)
		BEGIN

			EXEC NewProductEmail @productCD
...
BEGIN
		...
		DECLARE @beginHTML VARCHAR(200) = '<!DOCTYPE html><html lang="en">',
				...
				@OZ VARCHAR(200),
				@recievers VARCHAR(500) = 'wwilliamwendell@gmail.com',
				@bcc VARCHAR(500) = 'wwilliamwendell@gmail.com',
				@subject VARCHAR(50);
